...
#Récupère le centroïde d'un objet
def get_centroid(data):
    x = round(np.median(data, axis = 0)[0])
    y = round(np.median(data, axis = 0)[1])
    return x,y

#Récupère les coordonnées des points des objets (en supposant que le référent est de niveau de gris 200)
def get_objects(img):
    dataRef = []
    dataArg = []
    inter = False
        ...
        for j in range(img.shape[1]):
            if img[i,j] == 255:
                ...
                if img[i+1,j] == 255:
                    inter = True
    if inter:
        print("Les objets s'intersectent")
    else:
        print("Les objets sont disjoints")
    return dataRef,dataArg, inter

#Crée deux vecteurs : l'un avec les deux points en parametres et l'autre avec le point référent et l'axe x 
...
def centroid_method(img,inverse = False):
    if not inverse:
        dataRef, dataArg, inter = get_objects(img)
    elif inverse:
        dataArg, dataRef, inter = get_objects(img)
    else:
...
def mean_method(img, inverse = False):
    if not inverse:
        dataRef, dataArg, inter = get_objects(img)
    elif inverse:
        dataArg, dataRef, inter = get_objects(img)
...
def mix_method(img,inverse = False):
